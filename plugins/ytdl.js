...
cmd({
  ...
  try {
    ...
    let _0x206abf = "ZORAIB MD  WHATSAPP BOT„\n\n" + 
      ...
      ("*Url:* " + (_0x53641c || 'Unknown') + "\n\n") + 
      ('' + (config.FOOTER || "ZORAIB-MD Official“†ª"));
    ...
    // 🔥 YAHAN SIRF API CHANGE KI HAI 🔥
    // Pehle: const _0x567408 = await dy_scrap.ytmp3('https://youtube.com/watch?v=' + _0x82a833);
    // Ab: Arslan ki API use kar rahe hain
    
    const videoUrl = 'https://youtube.com/watch?v=' + _0x82a833;
    const apiUrl = `https://arslan-apis.vercel.app/download/ytmp3?url=${encodeURIComponent(videoUrl)}`;
    
    const response = await fetch(apiUrl);
    const _0x567408 = await response.json();
